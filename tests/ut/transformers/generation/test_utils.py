...
class GenerationTesterMixin:
    ...
    def _get_input_ids_and_config(self, batch_size=2):
        ...
        if "transfoxl" in config.__class__.__name__.lower():
            ...
            attention_mask = ops.ones_like(input_ids, dtype=mindspore.int64)[:batch_size, :sequence_length]
    ...
    def test_beam_search_generate_dict_output(self):
        for model_class in self.all_generative_model_classes:
            ...
            self.assertTrue(
                np.allclose(output_generate["sequences_scores"].asnumpy(), output_beam_search["sequences_scores"].asnumpy(), atol=1e-3)
            )
            self.assertTrue(output_generate["sequences_scores"].shape == (output_generate["sequences"].shape[0],))
            self.assertTrue((output_generate["sequences_scores"] < 0).all().asnumpy().item())
    ...
    def test_beam_sample_generate_dict_output(self):
        for model_class in self.all_generative_model_classes:
            ...
            self.assertTrue(
                np.allclose(output_generate["sequences_scores"].asnumpy(), output_beam_sample["sequences_scores"].asnumpy(), atol=1e-3)
            )
            self.assertTrue(output_generate["sequences_scores"].shape == (output_generate["sequences"].shape[0],))
            self.assertTrue((output_generate["sequences_scores"] < 0).all().asnumpy().item())
    ...
    def test_group_beam_search_generate_dict_output(self):
        for model_class in self.all_generative_model_classes:
            ...
            self.assertTrue(
                np.allclose(
                    output_generate["sequences_scores"].asnumpy(), output_group_beam_search["sequences_scores"].asnumpy(), atol=1e-3
                )
            )
            self.assertTrue(output_generate["sequences_scores"].shape == (output_generate["sequences"].shape[0],))
            self.assertTrue((output_generate["sequences_scores"] < 0).all().asnumpy().item())
    ...
    def test_generate_with_head_masking(self):
        ...
        for model_class in self.all_generative_model_classes:
            ...
            for attn_name, (name, mask) in zip(attention_names, head_masking.items()):
                ...
                self.assertEqual(sum([w.sum().asnumpy().item() for w in attn_weights]), 0.0)
    ...
    def _check_sequence_inside_sequence(self, tensor_1, tensor_2):
        ...
        if not isinstance(tensor_1, list):
            tensor_1 = tensor_1.asnumpy().tolist()
        if not isinstance(tensor_2, list):
            tensor_2 = tensor_2.asnumpy().tolist()
...
class GenerationIntegrationTests(MindNLPTestCase, GenerationIntegrationTestsMixin):
    ...
    def test_contrastive_search_batched(self):
        ...
        input_ids = tokenizer(articles[1], return_tensors="pt").input_ids
    ...
    def test_model_kwarg_encoder_signature_filtering(self):
        ...
        output = bart_model.generate(input_ids).asnumpy()
        ...
        fake_output = bart_model.generate(input_ids, foo="bar").asnumpy()
        ...
        fake_output = bart_model.generate(input_ids).asnumpy()
