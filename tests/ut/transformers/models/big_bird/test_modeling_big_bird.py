...
from mindnlp.transformers import BigBirdConfig
...
if is_mindspore_available():
    ...
    from mindnlp.transformers.models.big_bird.modeling_big_bird import (
...
class BigBirdModelTester:
    ...
    def create_and_check_for_pretraining(
        ...
        self.parent.assertEqual(
            result.seq_relationship_logits.shape, (self.batch_size, config.num_labels)
        )
...
class BigBirdModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    all_generative_model_classes = (
        (BigBirdForCausalLM,) if is_mindspore_available() else ()
    )
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(
            self, config_class=BigBirdConfig, hidden_size=37
        )
...
class BigBirdModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_block_sparse_pretraining(self):
        ...
        self.assertEqual(prediction_logits.shape, (1, 4096, 50358))
        self.assertEqual(seq_relationship_logits.shape, (1, 2))
        ...
        self.assertTrue(
            np.allclose(
                prediction_logits[0, 128:132, 128:132].asnumpy(),
                expected_prediction_logits_slice.asnumpy(),
                atol=1e-4,
            )
        )
        ...
        self.assertTrue(
            np.allclose(
                seq_relationship_logits.asnumpy(),
                expected_seq_relationship_logits.asnumpy(),
                atol=1e-4,
            )
        )
    ...
    def test_inference_full_pretraining(self):
        ...
        self.assertEqual(prediction_logits.shape, (1, 512 * 4, 50358))
        self.assertEqual(seq_relationship_logits.shape, (1, 2))
    ...
    def test_block_sparse_attention_probs(self):
        ...
        blocked_mask, band_mask, from_mask, to_mask = (
            model.create_masks_for_block_sparse_attn(attn_mask, config.block_size)
        )
        ...
        for i in range(config.num_hidden_layers):
            ...
            cl = cl.view(context_layer.shape)

            self.assertTrue(
                np.allclose(context_layer.asnumpy(), cl.asnumpy(), atol=0.001)
            )
    ...
    def test_block_sparse_context_layer(self):
        ...
        self.assertEqual(context_layer.shape, (1, 128, 768))
        self.assertTrue(
            np.allclose(
                context_layer[0, 64:78, 300:310].asnumpy(),
                targeted_cl.asnumpy(),
                atol=0.0001,
            )
        )
    ...
    def test_tokenizer_inference(self):
        ...
        self.assertEqual(prediction.shape, (1, 199, 768))
        ...
        self.assertTrue(
            np.allclose(
                prediction[0, 52:64, 320:324].asnumpy(),
                expected_prediction.asnumpy(),
                atol=1e-4,
            )
        )
    ...
    def test_inference_question_answering(self):
        ...
        target_start_logits = mindspore.tensor(
            [[-8.5622, -9.6209, -14.3351, -8.7032, -11.8596, -7.7446, -9.6730, -13.6063, -8.9651, -11.7417, -8.2641,
              -8.7056, -13.4116, -5.6600, -8.8316, -10.4148, -12.2180, -7.7979, -12.5274, -6.0685, -10.3373, -11.3128,
              -6.6456, -14.4030, -6.8292, -14.5383, -11.5638, -6.3326, 11.5293, -1.8434, -10.0013, -7.6150],
             [-10.7384, -13.1179, -10.1837, -13.7700, -10.0186, -11.7335, -13.3411, -10.0188, -13.4235, -9.9381,
              -10.4252, -13.1281, -8.2022, -10.4326, -11.5542, -14.1549, -10.7546, -13.4691, -8.2744, -11.4324,
              -13.3773, -9.8284, -14.5825, -8.7471, -14.7050, -8.0364, -11.3627, -6.4638, -11.7031, -14.3446, -9.9425,
              -8.0088]],  # noqa: E231

        )

        target_end_logits = mindspore.tensor(
            [[-12.1736, -8.8487, -14.8877, -11.6713, -15.1165, -12.2396, -7.6828, -15.4153, -12.2528, -14.3671,
              -12.3596, -7.4272, -14.9615, -13.6356, -11.7939, -9.9767, -14.8112, -8.9567, -15.8798, -11.5291, -9.4249,
              -14.7544, -7.9387, -16.2789, -8.9702, -15.3111, -11.5585, -7.9992, -4.1127, 10.3209, -8.3926, -10.2005],
             [-11.1375, -15.4027, -12.6861, -16.9884, -13.7093, -10.3560, -15.7228, -12.9290, -15.8519, -13.7953,
        ...
        self.assertTrue(
            np.allclose(
                ...
                target_start_logits.asnumpy(),
                atol=1e-4,
            )
        )
        self.assertTrue(
            np.allclose(
                end_logits[:, 64:96].asnumpy(), target_end_logits.asnumpy(), atol=1e-4
            )
        )
        ...
        answer = [
            input_ids[i][
                ops.argmax(start_logits, dim=-1)[i] : ops.argmax(end_logits, dim=-1)[i]
                + 1
            ]
    ...
    def test_fill_mask(self):
        ...
        pred_token = tokenizer.decode(ops.argmax(logits[0, 6:7], dim=-1))
    ...
    def test_auto_padding(self):
        ...
        input_ids = mindspore.tensor(
            [200 * [10] + 40 * [2] + [1]], dtype=mindspore.int64
        )
        # with torch.np():
        ...
        target = mindspore.tensor(
            [[-0.129420, -0.164740, 0.042422, -0.336030, 0.094379, 0.033794, 0.384590, 0.229660, -0.196500, 0.108020],
             [-0.000154, -0.168800, 0.165820, -0.313670, 0.101240, 0.035145, 0.381880, 0.213730, -0.201080, 0.077443],
             [0.053754, -0.166350, 0.225520, -0.272900, 0.119670, 0.019987, 0.348670, 0.199190, -0.181600, 0.084640],
             [0.063636, -0.187110, 0.237010, -0.297380, 0.126300, 0.020025, 0.268490, 0.191820, -0.192300, 0.035077],
             [0.073893, -0.184790, 0.188870, -0.297860, 0.134280, 0.028972, 0.174650, 0.186890, -0.180530, 0.006851],
             [0.005253, -0.169360, 0.123100, -0.302550, 0.126930, 0.024188, 0.133410, 0.200600, -0.168210, -0.001006],
             [-0.093336, -0.175370, -0.004768, -0.333170, 0.114330, 0.034168, 0.120960, 0.203570, -0.162810, -0.005757],
             [-0.160210, -0.169310, -0.049064, -0.331950, 0.115730, 0.027062, 0.143600, 0.205310, -0.144580, 0.026746],
             [-0.193200, -0.156820, -0.079422, -0.351600, 0.106450, 0.032174, 0.245690, 0.210250, -0.173480, 0.043914],
             [-0.167980, -0.153050, -0.059764, -0.357890, 0.103910, 0.031481, 0.334190, 0.208960, -0.178180, 0.072165],
             [-0.136990, -0.156950, -0.012099, -0.353140, 0.096996, 0.025864, 0.376340, 0.216050, -0.171820, 0.089963],
             [-0.041143, -0.167060, 0.079754, -0.353220, 0.093247, 0.019867, 0.385810, 0.214340, -0.191800, 0.065946],
             [0.040373, -0.158610, 0.152570, -0.312930, 0.110590, 0.012282, 0.345270, 0.204040, -0.176500, 0.064972],
             [0.043762, -0.166450, 0.179500, -0.317930, 0.117280, -0.004040, 0.304490, 0.201380, -0.182780, 0.044000]],
            # noqa: E231

        )
        # fmt: on

        self.assertEqual(output.shape, (1, 241, 768))

        self.assertTrue(
            np.allclose(
                output[0, 64:78, 300:310].asnumpy(), target.asnumpy(), atol=0.0001
            )
        )
