...
def _ranking_fast(
    context_hidden: mindspore.Tensor,
    next_hidden: mindspore.Tensor,
    next_top_k_probs: mindspore.Tensor,
    alpha: float,
    beam_width: int,
) -> mindspore.Tensor:
    """
    ...
    _, selected_idx = contrastive_score.max(axis=-1, return_indices=True)  # [B]
    return selected_idx
