        ...
        if self.position_embedding_type in ("relative_key", "relative_key_query"):
            ...
            if use_cache:
                position_ids_l = mindspore.Tensor(
                    key_length - 1, dtype=mindspore.int32
                ).view(-1, 1)
            else:
                position_ids_l = ops.arange(query_length, dtype=mindspore.int32).view(
                    -1, 1
                )
...
class SplinterFullyConnectedLayer(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_act="gelu"):
        ...
        self.dense = nn.Dense(self.input_dim, self.output_dim)
...
class QuestionAwareSpanSelectionHead(nn.Module):
    ...
    def forward(self, inputs, positions):
        ...
        start_logits = ops.matmul(hidden_states, start_reps)

        hidden_states = self.end_classifier(query_end_reps)
        end_reps = end_reps.permute(0, 2, 1)
        end_logits = ops.matmul(hidden_states, end_reps)
...
class SplinterForQuestionAnswering(SplinterPreTrainedModel):
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        question_positions: Optional[mindspore.Tensor] = None,
        ...
        end_positions (`mindspore.Tensor` of shape `(batch_size,)`, *optional*):
            ...
            Positions are clamped to the length of the sequence (`sequence_length`). Position outside of the sequence
            are not taken into account for computing the loss.
        question_positions (`mindspore.Tensor` of shape `(batch_size, num_questions)`, *optional*):
        ...
        if question_positions is None:
            if input_ids is not None:
                question_position_for_each_example = ops.argmax(
                    (ops.eq(input_ids, self.question_token_id)).int(), dim=-1
                )
            else:
                question_position_for_each_example = ops.zeros(
...
class SplinterForPreTrainingOutput(ModelOutput):
    ...
    Args:
        loss (`mindspore.Tensor` of shape `(1,)`, *optional*, returned when start and end positions are provided):
            Total span extraction loss is the sum of a Cross-Entropy for the start and end positions.
        start_logits (`mindspore.Tensor` of shape `(batch_size, num_questions, sequence_length)`):
            Span-start scores (before SoftMax).
        end_logits (`mindspore.Tensor` of shape `(batch_size, num_questions, sequence_length)`):
...
class SplinterForPreTraining(SplinterPreTrainedModel):
    ...
    def forward(
        ...
        r"""
        start_positions (`mindspore.Tensor` of shape `(batch_size, num_questions)`, *optional*):
            Labels for position (index) of the start of the labelled span for computing the token classification loss.
            Positions are clamped to the length of the sequence (`sequence_length`). Position outside of the sequence
            are not taken into account for computing the loss.
        end_positions (`mindspore.Tensor` of shape `(batch_size, num_questions)`, *optional*):
            Labels for position (index) of the end of the labelled span for computing the token classification loss.
            Positions are clamped to the length of the sequence (`sequence_length`). Position outside of the sequence
            are not taken into account for computing the loss.
        question_positions (`mindspore.Tensor` of shape `(batch_size, num_questions)`, *optional*):
        ...
        sequence_output = outputs[0]
        batch_size, sequence_length, dim = sequence_output.shape
        ...
        if attention_mask is not None:
            attention_mask_for_each_question = attention_mask.unsqueeze(1).broadcast_to(
                (batch_size, num_questions, sequence_length)
            )
            start_logits = start_logits + (
        ...
        if start_positions is not None and end_positions is not None:
            # sometimes the start/end positions are outside our model inputs, we ignore these terms
            start_positions.clamp(0, max(0, sequence_length - 1))
            end_positions.clamp(0, max(0, sequence_length - 1))
    ...
    def _prepare_question_positions(
        self, input_ids: mindspore.Tensor
    ) -> mindspore.Tensor:
        rows, flat_positions = np.where(input_ids == self.question_token_id)
        num_questions = np.bincount(rows, minlength=input_ids.shape[0])
        positions = np.full(
        ...
        # Efficiently create column indices for each question in each sequence
        cumulative_questions = np.cumsum(np.insert(num_questions, 0, 0))[:-1]
        cols = np.arange(len(flat_positions)) - cumulative_questions[rows]

        positions[rows, cols] = flat_positions
        return mindspore.Tensor(positions)
