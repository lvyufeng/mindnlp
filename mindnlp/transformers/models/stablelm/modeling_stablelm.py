...
class StableLmLayerNormPerHead(nn.Module):
    def __init__(self, dim, num_headseps=
1e-5, bias=False):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        self.norms = nn.ModuleList([nn.LayerNorm(dim, eps=
epsilon) for _ in range(self.num_heads)])
...
class StableLmAttention(nn.Module):
    ...
    def __init__(self, config: StableLmConfig, layer_idx: Optional[int] = None):
        ...
        self.q_proj = nn.Dense(self.hidden_size, self.num_heads * self.head_dim, bias=config.use_qkv_bias)
        self.k_proj = nn.Dense(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=config.use_qkv_bias)
        self.v_proj = nn.Dense(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=config.use_qkv_bias)
        self.o_proj = nn.Dense(self.hidden_size, self.hidden_size, bias=False)
