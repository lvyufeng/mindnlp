...
"""MindSpore Swin2SR Transformer model."""
...
def window_partition(input_feature, window_size):
    ...
    windows = input_feature.permute(0, 1, 3, 2, 4, 5).view(-1, window_size, window_size, num_channels)
    return windows
...
def window_reverse(windows, window_size, height, width):
    ...
    windows = windows.permute(0, 1, 3, 2, 4, 5).view(-1, height, width, num_channels)
    return windows
...
class Swin2SREmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.use_absolute_embeddings:
            self.position_embeddings = mindspore.Parameter(ops.zeros((1, num_patches + 1, config.embed_dim)), 'position_embeddings')
        else:
...
class Swin2SRPatchEmbeddings(nn.Module):
    def __init__(self, config, normalize_patches=True):
        ...
        self.layernorm = nn.LayerNorm([config.embed_dim], eps=
1e-5) if normalize_patches else None
...
class Swin2SRSelfAttention(nn.Module):
    def __init__(self, config, dim, num_heads, window_size, pretrained_window_size=[0, 0]):
        ...
        self.continuous_position_bias_mlp = nn.SequentialCell(
            nn.Dense(2, 512, bias=True), nn.ReLU(), nn.Dense(512, num_heads, bias=False)
        )

        # get relative_coords_table
        relative_coords_h = ops.arange(-(self.window_size[0] - 1), self.window_size[0], dtype=mindspore.int64).float()
        relative_coords_w = ops.arange(-(self.window_size[1] - 1), self.window_size[1], dtype=mindspore.int64).float()
        relative_coords_table = (
            ops.stack(meshgrid([relative_coords_h, relative_coords_w], indexing="ij"))
            .permute(1, 2, 0)
            .unsqueeze(0)
        ...
        relative_coords_table = (
            ops.sign(relative_coords_table) * ops.log2(ops.abs(relative_coords_table) + 1.0) / math.log2(8)
        )
        ...
        self.key = nn.Dense(self.all_head_size, self.all_head_size, bias=False)
    ...
    def forward(
        ...
        # cosine attention
        attention_scores = ops.norm(query_layer, dim=-1, keepdim=True) @ ops.norm(
            key_layer, dim=-1, keepdim=True
        ).swapaxes(-2, -1)
        logit_scale = ops.clamp(self.logit_scale, max=math.log(1.0 / 0.01)).exp()
...
class Swin2SRModel(Swin2SRPreTrainedModel):
    def __init__(self, config):
        ...
        if config.num_channels == 3 and config.num_channels_out == 3:
            ...
            self.mean = mindspore.Tensor(rgb_mean).view(1, 3, 1, 1)
        else:
    ...
    def pad_and_normalize(self, pixel_values):
        ...
        pixel_values = ops.pad(pixel_values, (0, modulo_pad_width, 0, modulo_pad_height), "reflect")
...
class NearestConvUpsampler(nn.Module):
    ...
    def forward(self, sequence_output):
        ...
        sequence_output = self.lrelu(
            # TODO:self.conv_up1(ops.interpolate(sequence_output, scale_factor=2.0, mode="nearest", recompute_scale_factor=True))
            self.conv_up1(ops.interpolate(sequence_output, scale_factor=2, mode="nearest", recompute_scale_factor=True))
        )
        sequence_output = self.lrelu(
...
class Swin2SRForImageSuperResolution(Swin2SRPreTrainedModel):
    ...
    def forward(
        ...
        if self.config.upsampler == "pixelshuffle_aux":
            # TODO: the results may not align
            bicubic = ops.interpolate(
                pixel_values,
