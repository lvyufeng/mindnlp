...
class Mask2FormerMaskedAttentionDecoderOutput(BaseModelOutputWithCrossAttentions):
    ...
    Args:
        ...
        masks_queries_logits (`tuple(mindspore.Tensor)` of shape `(batch_size, num_queries, height, width)`):
            Tuple of mask predictions from all layers of the transformer decoder.
        intermediate_hidden_states (`tuple(mindspore.Tensor)` of shape `(num_queries, 1, hidden_size)`):
    ...
    attentions: Optional[mindspore.Tensor] = None
    masks_queries_logits: Tuple[mindspore.Tensor] = None
    intermediate_hidden_states: Tuple[mindspore.Tensor] = None
...
class Mask2FormerPixelLevelModuleOutput(ModelOutput):
    ...
    Args:
        encoder_last_hidden_state (`mindspore.Tensor`):
            Last hidden states (final feature map of shape `(batch_size, num_channels, height, width)`) of the last
            stage of the encoder.
        encoder_hidden_states (`tuple(mindspore.Tensor)`, *optional*):
            Tuple of `mindspore.Tensor` of shape `(batch_size, num_channels, height, width)`. Hidden states (also
            called feature maps) of the model at the output of each stage. Returned if output_hidden_states is set to
            True.
        decoder_last_hidden_state (`mindspore.Tensor` of shape `(batch_size, num_channels, height, width)):
    ...
    encoder_last_hidden_state: mindspore.Tensor = None
    encoder_hidden_states: Optional[Tuple[mindspore.Tensor]] = None
    decoder_last_hidden_state: mindspore.Tensor = None
    decoder_hidden_states: Tuple[mindspore.Tensor] = None
...
class Mask2FormerModelOutput(ModelOutput):
    ...
    Args:
        encoder_last_hidden_state (`mindspore.Tensor` of shape `(batch_size, num_channels, height, width)`, *optional*):
        ...
        pixel_decoder_last_hidden_state (`mindspore.Tensor` of shape `(batch_size, num_channels, height, width)`, *optional*):
        ...
        transformer_decoder_last_hidden_state (`tuple(mindspore.Tensor)`):
            Final output of the transformer decoder `(batch_size, sequence_length, hidden_size)`.
        transformer_decoder_hidden_states (`tuple(mindspore.Tensor)`, *optional*):
            Tuple of `mindspore.Tensor` (one for the output of the embeddings + one for the output of each stage) of
            shape `(batch_size, sequence_length, hidden_size)`. Hidden-states (also called feature maps) of the
            transformer decoder at the output of each stage. Returned when `output_hidden_states=True` is passed.
        transformer_decoder_intermediate_states (`tuple(mindspore.Tensor)` of shape `(num_queries, 1, hidden_size)`):
            Intermediate decoder activations, i.e. the output of each decoder layer, each of them gone through a
            layernorm.
        masks_queries_logits (`tuple(mindspore.Tensor)` of shape `(batch_size, num_queries, height, width)`)
            Mask Predictions from each layer in the transformer decoder.
        attentions (`tuple(tuple(mindspore.Tensor))`, *optional*, returned when `output_attentions=True` is passed):
    ...
    encoder_last_hidden_state: mindspore.Tensor = None
    pixel_decoder_last_hidden_state: mindspore.Tensor = None
    transformer_decoder_last_hidden_state: mindspore.Tensor = None
    encoder_hidden_states: Optional[Tuple[mindspore.Tensor]] = None
    pixel_decoder_hidden_states: Optional[Tuple[mindspore.Tensor]] = None
    transformer_decoder_hidden_states: Optional[Tuple[mindspore.Tensor]] = None
    transformer_decoder_intermediate_states: Tuple[mindspore.Tensor] = None
    masks_queries_logits: Tuple[mindspore.Tensor] = None
    attentions: Optional[Tuple[mindspore.Tensor]] = None
...
class Mask2FormerForUniversalSegmentationOutput(ModelOutput):
    ...
    Args:
        ...
        masks_queries_logits (`mindspore.Tensor`):
            ...
            query.
        auxiliary_logits (`List[Dict(str, mindspore.Tensor)]`, *optional*):
            List of class and mask predictions from each layer of the transformer decoder.
        encoder_last_hidden_state (`mindspore.Tensor` of shape `(batch_size, num_channels, height, width)`):
        ...
        pixel_decoder_last_hidden_state (`mindspore.Tensor` of shape `(batch_size, num_channels, height, width)`):
            Last hidden states (final feature map) of the last stage of the pixel decoder model.
        pixel_decoder_hidden_states (`tuple(mindspore.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            Tuple of `mindspore.Tensor` (one for the output of the embeddings + one for the output of each stage) of
            shape `(batch_size, num_channels, height, width)`. Hidden-states (also called feature maps) of the pixel
    ...
    auxiliary_logits: Optional[List[Dict[str, mindspore.Tensor]]] = None
    encoder_last_hidden_state: mindspore.Tensor = None
    pixel_decoder_last_hidden_state: mindspore.Tensor = None
    transformer_decoder_last_hidden_state: mindspore.Tensor = None
    encoder_hidden_states: Optional[Tuple[mindspore.Tensor]] = None
...
def sample_point(
    ...
    """
    A wrapper around `ops.ops.grid_sample` to support 3D point_coordinates tensors.
...
def dice_loss(inputs: mindspore.Tensor, labels: mindspore.Tensor, num_masks: int) -> mindspore.Tensor:
    r"""
...
def pair_wise_dice_loss(inputs: mindspore.Tensor, labels: mindspore.Tensor) -> mindspore.Tensor:
    """
...
class Mask2FormerHungarianMatcher(nn.Module):
    ...
    def forward(
        ...
        for i in range(batch_size):
            pred_probs = ops.softmax(class_queries_logits[i], -1)
            ...
            target_mask = mask_labels[i].to(pred_mask.dtype)
            ...
            cost_matrix = ops.minimum(cost_matrix, mindspore.tensor(1e10))
            cost_matrix = ops.maximum(cost_matrix, mindspore.tensor(-1e10))
...
class Mask2FormerLoss(nn.Module):
    def __init__(self, config: Mask2FormerConfig, weight_dict: Dict[str, float]):
        ...
        empty_weight = ops.ones(self.num_labels + 1)
    ...
    def _pad_images_to_max_in_batch(self, tensors: List[mindspore.Tensor]) -> Tuple[mindspore.Tensor, mindspore.Tensor]:
        ...
        padding_masks = ops.ones((batch_size, height, width), dtype=mindspore.bool_)
    ...
    def loss_labels(
        self, class_queries_logits: mindspore.Tensor, class_labels: List[mindspore.Tensor], indices: Tuple[np.array]
        ...
        target_classes = ops.full(
            (batch_size, num_queries), fill_value=self.num_labels, dtype=mindspore.int32
        )
    ...
    def loss_masks(
        self,
        masks_queries_logits: mindspore.Tensor,
        mask_labels: List[mindspore.Tensor],
        indices: Tuple[np.array],
        num_masks: int,
    ) -> Dict[str, mindspore.Tensor]:
        ...
        Returns:
            losses (`Dict[str, Tensor]`): A dict of `mindspore.Tensor` containing two keys:
    ...
    def forward(
        ...
        mask_labels: List[mindspore.Tensor],
        class_labels: List[mindspore.Tensor],
        auxiliary_predictions: Optional[Dict[str, mindspore.Tensor]] = None,
    ) -> Dict[str, mindspore.Tensor]:
        """
        ...
        Returns:
            losses (`Dict[str, Tensor]`): A dict of `mindspore.Tensor` containing three keys:
            - **loss_cross_entropy** -- The loss computed using cross entropy on the predicted and ground truth labels.
        ...
        # get all the losses
        losses: Dict[str, mindspore.Tensor] = {
...
def multi_scale_deformable_attention(
    value: mindspore.Tensor, value_spatial_shapes: mindspore.Tensor, sampling_locations: mindspore.Tensor, attention_weights: mindspore.Tensor
) -> mindspore.Tensor:
...
class Mask2FormerSinePositionEmbedding(nn.Module):
    ...
    def forward(self, x: mindspore.Tensor, mask: Optional[mindspore.Tensor] = None) -> mindspore.Tensor:
        if mask is None:
...
class Mask2FormerPixelDecoderEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        if output_attentions:
            outputs += (attn_weights.swapaxes(1, 0),)
    ...
    def forward(
        ...
        for i, encoder_layer in enumerate(self.layers):
            if output_hidden_states:
                all_hidden_states += (hidden_states.swapaxes(1, 0),)
...
class Mask2FormerPixelDecoder(nn.Module):
    def __init__(self, config: Mask2FormerConfig, feature_channels):
        ...
        self.level_embed = Parameter(ops.zeros([self.num_feature_levels, feature_dim]))
    ...
    def forward(
        ...
        masks = [
            ops.zeros((x.shape[0], x.shape[2], x.shape[3]), dtype=mindspore.bool_) for x in input_embeds
        ]
        ...
        outputs = [
            x.swapaxes(1, 2).view(batch_size, -1, spatial_shapes[i][0].item(), spatial_shapes[i][1].item())
...
class Mask2FormerPixelLevelModule(nn.Module):
    ...
    def forward(self, pixel_values: mindspore.Tensor, output_hidden_states: bool = False) -> Mask2FormerPixelLevelModuleOutput:
...
class Mask2FormerMaskedAttentionDecoderLayer(nn.Module):
    ...
    def __init__(self, config: Mask2FormerConfig):
        ...
        self.fc1 = nn.Dense(self.embed_dim, self.config.dim_feedforward)
        self.fc2 = nn.Dense(self.config.dim_feedforward, self.embed_dim)
        self.final_layer_norm = nn.LayerNorm(self.embed_dim)

    def with_pos_embed(self, tensor, pos: Optional[mindspore.Tensor]):
    ...
    def forward_post(
        ...
        hidden_states, cross_attn_weights = self.cross_attn(
            self.with_pos_embed(hidden_states, query_position_embeddings),
            self.with_pos_embed(encoder_hidden_states[level_index], position_embeddings[level_index]),
...
class Mask2FormerMaskedAttentionDecoder(nn.Module):
    ...
    def forward(
        ...
        multi_stage_positional_embeddings: mindspore.Tensor = None,
        pixel_embeddings: mindspore.Tensor = None,
        encoder_hidden_states: mindspore.Tensor = None,
        query_position_embeddings: mindspore.Tensor = None,
        ...
        r"""
        Args:
            inputs_embeds (`mindspore.Tensor` of shape `(num_queries, batch_size, hidden_size)`):
                The query embeddings that are passed into the decoder.
            multi_stage_positional_embeddings (`mindspore.Tensor` of shape `(height*width, batch_size, num_channels)`):
                Position embeddings that are added to the keys in each cross(masked)-attention layer.
            pixel_embeddings (`mindspore.Tensor`):
                Tensor of shape `(batch_size, num_channels, height, width)`, 1/4 scale features from the last Pixel
                Decoder.
            query_position_embeddings (`mindspore.Tensor` of shape `(num_queries, batch_size, hidden_size)`):
            ...
            feature_size_list (`List[ops.Size]` ):
        ...
        hidden_states = hidden_states.swapaxes(1, 0)
        if not return_dict:
...
class Mask2FormerMaskPredictor(nn.Module):
    def __init__(self, hidden_size: int, num_heads: int, mask_feature_size: mindspore.Tensor):
        """
        ...
        Args:
            ...
            mask_feature_size (`mindspore.Tensor`):
    ...
    def forward(self, outputs: mindspore.Tensor, pixel_embeddings: mindspore.Tensor, attention_mask_target_size: int = None):
        mask_embeddings = self.mask_embedder(outputs.swapaxes(0, 1))
...
class Mask2FormerTransformerModule(nn.Module):
    ...
    def forward(
        self,
        multi_scale_features: List[mindspore.Tensor],
        mask_features: mindspore.Tensor,
...
class Mask2FormerForUniversalSegmentation(Mask2FormerPreTrainedModel):
    ...
    def __init__(self, config: Mask2FormerConfig):
        ...
        self.class_predictor = nn.Dense(config.hidden_dim, config.num_labels + 1)
    ...
    def get_loss_dict(
        ...
        mask_labels: mindspore.Tensor,
        class_labels: mindspore.Tensor,
        auxiliary_predictions: Dict[str, mindspore.Tensor],
    ) -> Dict[str, mindspore.Tensor]:
        loss_dict: Dict[str, mindspore.Tensor] = self.criterion(
    ...
    def get_loss(self, loss_dict: Dict[str, mindspore.Tensor]) -> mindspore.Tensor:
        return sum(loss_dict.values())

    def get_auxiliary_logits(self, classes: mindspore.Tensor, output_masks: mindspore.Tensor):
        auxiliary_logits: List[Dict[str, mindspore.Tensor]] = []
    ...
    def forward(
        self,
        pixel_values: mindspore.Tensor,
        mask_labels: Optional[List[mindspore.Tensor]] = None,
        class_labels: Optional[List[mindspore.Tensor]] = None,
        pixel_mask: Optional[mindspore.Tensor] = None,
        output_hidden_states: Optional[bool] = None,
        ...
        for decoder_output in outputs.transformer_decoder_intermediate_states:
            class_prediction = self.class_predictor(decoder_output.swapaxes(0, 1))
