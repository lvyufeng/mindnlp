...
class XCLIPCrossAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.q_proj = nn.Dense(dim, dim, bias=False)
        self.k_proj = nn.Dense(dim, dim, bias=False)
        self.v_proj = nn.Dense(dim, dim, bias=False)
...
class PromptGeneratorLayer(nn.Module):
    def __init__(self, config):
        ...
        self.norm1 = nn.LayerNorm(
            embed_dim, eps=
config.text_config.layer_norm_eps)
