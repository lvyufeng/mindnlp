...
class GroupViTModel(GroupViTPreTrainedModel):
    ...
    def __init__(self, config: GroupViTConfig):
        ...
        self.visual_projection = nn.SequentialCell(
            nn.Dense(self.vision_embed_dim, self.projection_intermediate_dim, bias=True),
            nn.BatchNorm1d(self.projection_intermediate_dim),
            nn.ReLU(),
            nn.Dense(self.projection_intermediate_dim, self.projection_dim, bias=True),
        )
        self.text_projection = nn.SequentialCell(
            nn.Dense(self.text_embed_dim, self.projection_intermediate_dim, bias=True),
            nn.BatchNorm1d(self.projection_intermediate_dim),
            nn.ReLU(),
            nn.Dense(self.projection_intermediate_dim, self.projection_dim, bias=True),
        )
