...
class MusicgenMelodyProcessor(ProcessorMixin):
    ...
    def __call__(self, audio=None, text=None, **kwargs):
        ...
        Args:
            audio (`np.ndarray`, `torch.Tensor`, `List[np.ndarray]`, `List[torch.Tensor]`):
                ...
                of a NumPy array/MindSpore tensor, each audio should be a mono-stereo signal of shape (T), where T is the sample length of the audio.
    ...
    def _decode_audio(self, audio_values, attention_mask = None) -> List[np.ndarray]:
        """
    ...
    def get_unconditional_inputs(self, num_samples=1, return_tensors="ms"):
        """
