...
def window_partition(hidden_states, window_size):
    ...
    Args:
        hidden_states (`mindspore.Tensor` of shape `(batch_size, height, width, num_channels)`):
            Input hidden states
        window_size (`int`):
            Window size
    """
    ...
    windows = hidden_states.permute(0, 1, 3, 2, 4, 5).view(-1, window_size, window_size, num_channels)
    return windows
...
def window_reverse(windows, window_size, height, width):
    ...
    Args:
        windows (`mindspore.Tensor` of shape `(num_windows * batch_size, window_size, window_size, num_channels)`):
            Input windows
        window_size (`int`):
            Window size
        height (`int`):
...
class ClapAudioModelOutput(ModelOutput):
    ...
    Args:
        audio_embeds (`mindspore.Tensor` of shape `(batch_size, hidden_size)`):
    ...
    audio_embeds: Optional[mindspore.Tensor] = None
    last_hidden_state: mindspore.Tensor = None
    hidden_states: Optional[Tuple[mindspore.Tensor, ...]] = None
    attentions: Optional[Tuple[mindspore.Tensor, ...]] = None
...
class ClapOutput(ModelOutput):
    ...
    logits_per_audio: mindspore.Tensor = None
    logits_per_text: mindspore.Tensor = None
    text_embeds: mindspore.Tensor = None
    audio_embeds: mindspore.Tensor = None
...
class ClapAudioEncoder(nn.Module):
    ...
    def reshape_mel2img(self, normalized_input_features):
        ...
        normalized_input_features = normalized_input_features.permute(0, 1, 3, 2)
        normalized_input_features = normalized_input_features.reshape(
    ...
    def forward(
        ...
        last_hidden_state = (
            last_hidden_state.permute(0, 2, 1).reshape(batch_size, n_channels, freq_shape, temporal_shape)
        )
...
class ClapModel(ClapPreTrainedModel):
    ...
    def get_audio_features(
        ...
        r"""
        Returns:
            audio_features (`mindspore.Tensor` of shape `(batch_size, output_dim`): The audio embeddings obtained by
