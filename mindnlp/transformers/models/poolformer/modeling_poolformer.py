...
def drop_path(input: ms.Tensor, drop_prob: float = 0.0, training: bool = False) -> ms.Tensor:
    ...
    random_tensor = keep_prob + \
        ops.rand(shape, dtype=input.dtype)
...
class PoolFormerEmbeddings(nn.Module):
    ...
    def __init__(self, hidden_size, num_channels, patch_size, stride, padding, norm_layer=None):
        ...
        stride = stride if isinstance(
            stride, collections.abc.Iterable) else (stride, stride)
        padding = padding if isinstance(
...
class PoolFormerLayer(nn.Module):
    ...
    def __init__(self, config, num_channels, pool_size, hidden_size, intermediate_size, drop_path):
        ...
        self.drop_path = PoolFormerDropPath(
    ...
    def forward(self, hidden_states):
        if self.use_layer_scale:
            ...
            layer_output = self.drop_path(
                self.output(self.after_norm(hidden_states)))
