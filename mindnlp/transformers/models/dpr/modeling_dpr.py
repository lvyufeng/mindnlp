...
class DPREncoder(DPRPreTrainedModel):
    ...
    def __init__(self, config: DPRConfig):
        ...
        if self.projection_dim > 0:
            self.encode_proj = nn.Dense(self.bert_model.config.hidden_size, config.projection_dim, bias=True)
...
class DPRSpanPredictor(DPRPreTrainedModel):
    ...
    def __init__(self, config: DPRConfig):
        ...
        self.qa_outputs = nn.Dense(self.encoder.embeddings_size, 2, bias=True)
        self.qa_classifier = nn.Dense(self.encoder.embeddings_size, 1, bias=True)
        # Initialize weights and apply final processing
        self.post_init()
