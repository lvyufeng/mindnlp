...
def flatten_01(x:Tensor) -> Tensor:
    '''implement flatten(start_dim=0, end_dim=1)'''
    B, C, *others = x.shape
...
class DetrSegmentationOutput(ModelOutput):
    ...
    Args:
        ...
        pred_boxes (`mindspore.Tensor` of shape `(batch_size, num_queries, 4)`):
            ...
            possible padding). You can use [`~DetrImageProcessor.post_process_object_detection`] to retrieve the
            unnormalized bounding boxes.
        pred_masks (`mindspore.Tensor` of shape `(batch_size, num_queries, height/4, width/4)`):
    ...
    def __init__(self, config: DetrConfig):
        ...
        if config.use_timm_backbone:
            if is_timm_available:
                from timm import create_model       # pylint: disable=import-error
                ...
                if backbone_config is None:
                    ...
                    backbone_model_type = 'resnet'
                    backbone_config = CONFIG_MAPPING[backbone_model_type](out_features=["stage4"])
                elif isinstance(backbone_config, dict):
                    backbone_model_type = backbone_config.get("model_type")
                    config_class = CONFIG_MAPPING[backbone_model_type]
                    backbone_config = config_class.from_dict(backbone_config)
                config.backbone = backbone_model_type
...
class DetrDecoder(DetrPreTrainedModel):
    ...
    def forward(
        ...
        if self.config.auxiliary_loss:
            intermediate = ops.stack(intermediate)
...
class DetrModel(DetrPreTrainedModel):
    ...
    ) -> Union[Tuple[mindspore.Tensor], DetrModelOutput]:
        r"""
        Returns:
...
class DetrForObjectDetection(DetrPreTrainedModel):
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[mindspore.Tensor], DetrObjectDetectionOutput]:
        r"""
...
class DetrForSegmentation(DetrPreTrainedModel):
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[mindspore.Tensor], DetrSegmentationOutput]:
        r"""
        labels (`List[Dict]` of len `(batch_size,)`, *optional*):
            Labels for computing the bipartite matching loss, DICE/F-1 loss and Focal loss. List of dicts, each
            dictionary containing at least the following 3 keys: 'class_labels', 'boxes' and 'masks' (the class labels,
            bounding boxes and segmentation masks of an image in the batch respectively). The class labels themselves
            should be a `mindspore.Tensor` of len `(number of bounding boxes in the image,)`, the boxes a
            `mindspore.Tensor` of shape `(number of bounding boxes in the image, 4)` and the masks a
            `mindspore.Tensor` of shape `(number of bounding boxes in the image, height, width)`.
...
def _expand(tensor, length: int):
    return flatten_01(tensor.unsqueeze(1).repeat(1, int(length), 1, 1, 1))
...
class DetrMaskHeadSmallConv(nn.Module):
    ...
    def forward(self, x: Tensor, bbox_mask: Tensor, fpns: List[Tensor]):
        ...
        # TODO: impl flatten01
        x = ops.cat([_expand(x, bbox_mask.shape[1]), flatten_01(bbox_mask)], 1)
...
class DetrMHAttentionMap(nn.Module):
    ...
    def __init__(self, query_dim, hidden_dim, num_heads, dropout=0.0, bias=True, std=None):
        ...
        self.q_linear = nn.Dense(query_dim, hidden_dim, bias=bias)
        self.k_linear = nn.Dense(query_dim, hidden_dim, bias=bias)
    ...
    def forward(self, q, k, mask: Optional[Tensor] = None):
        ...
        if mask is not None:
            weights = ops.masked_fill(weights, mask.unsqueeze(1).unsqueeze(1), finfo(weights.dtype, 'min'))
...
class DetrMLPPredictionHead(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers):
        ...
        self.layers = nn.ModuleList([nn.Dense(n, k) for n, k in zip([input_dim] + h, h + [output_dim])])
...
class DetrHungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        out_prob = ops.softmax(flatten_01(outputs["logits"]), -1)  # [batch_size * num_queries, num_classes]
        out_bbox = flatten_01(outputs["pred_boxes"])               # [batch_size * num_queries, 4]
