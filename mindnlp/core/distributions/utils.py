...
from mindnlp.core import Tensor, tensor
...
def broadcast_all(*values):
    ...
    """
    if not all(isinstance(v, Tensor) or isinstance(v, Number) for v in values):
        raise ValueError(
            "Input arguments must all be instances of numbers.Number, "
            "Tensor or objects implementing __torch_function__."
        )
    if not all(isinstance(v, Tensor) for v in values):
        ...
        for value in values:
            if isinstance(value, Tensor):
                ...
                break
        new_values = [
            v if isinstance(v, Tensor) else tensor(v, **options) for v in values
        ]
...
def logits_to_probs(logits, is_binary=False):
    ...
    return ops.softmax(logits, dim=-1)
...
def vec_to_tril_matrix(vec: Tensor, diag: int = 0) -> Tensor:
    ...
    n = round(n.item()) if isinstance(n, Tensor) else round(n)
